...
class ConnectSite extends Component
{
    ...
    public function mount(): void
    {
        ...
        // "Add site" from a client page carries the client in the query string;
        // dropping it silently filed the new site under no client at all.
    ...
    /**
     * Register the site before it can answer — the escape hatch out of a
     * circular dependency.
     *
     * The credentials this screen asks for are generated BY the connector, on
     * the site, at activation. Someone who does not have WordPress access yet
     * (or is waiting on the client for it) could otherwise not record the site
     * at all, because {@see connect()} refuses anything the connector has not
     * confirmed.
     *
     * What is created is deliberately inert: `is_connected` stays false and no
     * sync is dispatched, so nothing is scanned, backed up or monitored until
     * the credentials are pasted into the connect dialog on the site page. That
     * is the difference from the old wizard, which produced sites that looked
     * connected and silently never were.
     */
    public function saveWithoutConnecting(): void
    {
        abort_unless((bool) auth()->user()?->canManageSites(), 403, 'Viewers cannot connect sites.');
        ...
        $site = Site::create([
            ...
            'type' => 'wordpress',
            'status' => 'pending',
            // Whatever was typed is kept, but an unverified pair is not enough
            // to call the site connected — the connect dialog verifies on save.
            'api_key' => $this->apiKey !== '' ? $this->apiKey : null,
            'api_secret' => $this->apiSecret !== '' ? $this->apiSecret : null,
            'api_endpoint' => rtrim($this->url, '/').'/wp-json/simplead/v1',
        ]);
        ...
        session()->flash('message', __('":name" saved but not connected yet — paste the key and secret to finish.', ['name' => $site->name]));

        // ?connect=1 opens the credentials dialog on arrival, so the next step
        // is in front of you instead of behind a settings icon.
        $this->redirect(route('sites.overview', $site).'?connect=1', navigate: true);
    }
