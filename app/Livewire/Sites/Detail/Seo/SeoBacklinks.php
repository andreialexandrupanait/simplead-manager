...
class SeoBacklinks extends Component
{
    ...
    public function exportDisavow()
    {
        $toxicDomains = Backlink::where('site_id', $this->site->id)
            ->active()
            ->toxic()
            ->where('source_domain', '!=', 'gsc-aggregate')
        ...
        if (empty($toxicDomains)) {
            Session::flash('error', __('No toxic backlinks found to disavow.'));

            return;
        }

        $filename = 'disavow-'.$this->site->slug.'-'.now()->format('Y-m-d').'.txt';

        return response()->streamDownload(function () use ($toxicDomains) {
            echo "# Disavow file generated by SimpleAd Manager\n";
            echo '# Site: '.$this->site->url."\n";
            echo '# Date: '.now()->format('Y-m-d H:i')."\n";
            echo '# Toxic domains (spam score >= 60): '.count($toxicDomains)."\n\n";

            foreach ($toxicDomains as $domain) {
                echo "domain:{$domain}\n";
            }
