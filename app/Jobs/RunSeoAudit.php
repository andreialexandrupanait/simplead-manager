...
class RunSeoAudit implements ShouldBeUnique, ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        try { $r = Http::timeout(15)->withHeaders(['X-SAM-API-Key' => $this->site->api_key ?? ''])->get(rtrim($this->site->url, '/').'/wp-json/simplead/v1/seo/analysis'); if ($r->successful()) { $d = $r->json('data', []); $u = []; if (isset($d['seo_plugin'])) { $u['seo_plugin'] = $d['seo_plugin']['name'] ?? null; $u['seo_plugin_version'] = $d['seo_plugin']['version'] ?? null; } if (!empty($u)) $this->audit->update($u); } } catch (\Throwable $e) { Log::debug('SEO: connector fetch failed', ['error' => $e->getMessage()]); }
