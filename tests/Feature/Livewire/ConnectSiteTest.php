...
class ConnectSiteTest extends TestCase
{
    ...
    /**
     * The credentials are generated BY the connector, on the site. Requiring
     * them to register the site at all made the screen a closed loop for anyone
     * without WordPress access yet.
     */
    public function test_a_site_can_be_registered_before_the_connector_exists(): void
    {
        Queue::fake();

        Livewire::test(ConnectSite::class)
            ->set('url', 'https://not-yet.test')
            ->set('name', 'Not Yet')
            ->call('saveWithoutConnecting')
            ->assertHasNoErrors();

        $site = Site::where('url', 'https://not-yet.test')->firstOrFail();
        $this->assertSame('pending', $site->status);
        $this->assertFalse($site->is_connected);
        $this->assertNull($site->api_key);
        $this->assertNull($site->api_secret);
    }

    /**
     * An unpaired site must stay inert: syncing it would scan, back up and
     * monitor a site that cannot answer. That was the old wizard's defect.
     */
    public function test_registering_without_the_connector_starts_no_sync(): void
    {
        Queue::fake();

        Livewire::test(ConnectSite::class)
            ->set('url', 'https://inert.test')
            ->set('name', 'Inert')
            ->call('saveWithoutConnecting');
    ...
    public function test_registering_without_the_connector_still_needs_a_url_and_name(): void
    {
        Livewire::test(ConnectSite::class)
            ->set('url', '')
            ->set('name', '')
            ->call('saveWithoutConnecting')
            ->assertHasErrors(['url', 'name']);
    }

    public function test_a_viewer_cannot_register_a_site_either(): void
    {
        $this->actingAs(User::factory()->create(['role' => UserRole::Viewer]));

        Livewire::test(ConnectSite::class)
            ->set('url', 'https://viewer-register.test')
            ->set('name', 'Viewer')
            ->call('saveWithoutConnecting')
            ->assertForbidden();
    }
    ...
    /**
     * "Add site" from a client page carries ?client_id=; dropping it filed the
     * new site under no client at all.
     */
    public function test_the_client_from_the_query_string_is_preselected(): void
    {
        $client = Client::factory()->create(['status' => 'active']);

        Livewire::withQueryParams(['client_id' => $client->id])
            ->test(ConnectSite::class)
            ->assertSet('clientId', $client->id);
    }

    public function test_an_unknown_client_in_the_query_string_is_ignored(): void
    {
        Livewire::withQueryParams(['client_id' => 999999])
            ->test(ConnectSite::class)
            ->assertSet('clientId', null);
    }

    /**
     * The screen states what to install — it must also say where to get it,
     * or the credentials it asks for are unobtainable.
     */
    public function test_the_screen_links_to_the_connector_download(): void
    {
        Livewire::test(ConnectSite::class)
            ->assertSee(route('download.connector-plugin'), false)
            ->assertSee('SAD Mentenanta');
    }
}
