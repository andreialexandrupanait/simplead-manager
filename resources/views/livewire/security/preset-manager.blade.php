<div>
    <x-ui.page-header title="Security Presets" subtitle="Manage reusable security hardening configurations">
        <x-ui.button wire:click="$set('showForm', true)" size="sm">
            <x-icons.plus class="h-4 w-4 mr-1" /> New Preset
        </x-ui.button>
    </x-ui.page-header>
    ...
    @if($showForm)
        <x-ui.card class="mb-6">
            ...
            <div class="space-y-4">
                <x-ui.form-group label="Name" for="presetName" error="{{ $errors->first('presetName') }}">
                    <x-ui.input type="text" id="presetName" wire:model="presetName" placeholder="e.g. Maximum Security" />
                </x-ui.form-group>

                <x-ui.form-group label="Description" for="presetDescription">
                    <textarea id="presetDescription" wire:model="presetDescription" rows="2"
                        ...
                        placeholder="Describe what this preset configures..."></textarea>
                </x-ui.form-group>

                <label class="flex items-center gap-2">
                    <x-ui.checkbox wire:model="isDefault" />
                    <span class="text-sm text-gray-700">Set as default preset</span>
                </label>

                <div class="flex gap-2 justify-end">
                    <x-ui.button variant="secondary" wire:click="resetForm">Cancel</x-ui.button>
    ...
    {{-- Create From Site --}}
    <x-ui.card class="mb-6">
        <h4 class="text-sm font-semibold text-gray-900 mb-3">Create Preset from Site</h4>
        <div class="flex flex-wrap items-end gap-3">
            <div class="w-48">
                <x-ui.form-group label="Source Site" for="snapshotSiteId" error="{{ $errors->first('snapshotSiteId') }}">
            ...
            <div class="flex-1 min-w-[200px]">
                <x-ui.form-group label="Preset Name" for="snapshotName" error="{{ $errors->first('snapshotName') }}">
                    <x-ui.input type="text" id="snapshotName" wire:model="snapshotName" placeholder="Preset name" class="text-sm" />
                </x-ui.form-group>
            </div>
    ...
    <x-ui.card>
        @if($this->presets->isEmpty())
            <x-ui.empty-state
                ...
                description="Create a preset or run the seeder to get started."
                icon="shield"
            />
        @else
            <div class="space-y-4">
                    ...
                    <div class="rounded-lg border border-gray-200 p-4">
                        <div class="flex items-start justify-between">
                            <div>
                                ...
                                <p class="mt-1 text-xs text-gray-400">
                                    {{ count($preset->settings ?? []) }} categories &middot;
                                    {{ $preset->sites_count }} site(s) applied
                                </p>
                            </div>
                            <div class="flex gap-1">
                                ...
                                <x-ui.button variant="secondary" size="sm" wire:click="deletePreset({{ $preset->id }})"
                                    wire:confirm="Delete preset '{{ $preset->name }}'?">
                        ...
                        @if($applyingPresetId === $preset->id)
                            <div class="mt-4 border-t border-gray-100 pt-4">
                                <h5 class="text-sm font-medium text-gray-900 mb-2">Select sites to apply:</h5>
                                ...
                                <div class="flex gap-2">
                                    <x-ui.button size="sm" wire:click="applyToSites"
                                        wire:confirm="Apply '{{ $preset->name }}' to {{ count($applySiteIds) }} site(s)?"
                                        wire:loading.attr="disabled">
                                        Apply to {{ count($applySiteIds) }} site(s)
                                    </x-ui.button>
                                    <x-ui.button variant="secondary" size="sm" wire:click="cancelApply">Cancel</x-ui.button>
                                </div>
                            </div>
                        @endif
                    </div>
