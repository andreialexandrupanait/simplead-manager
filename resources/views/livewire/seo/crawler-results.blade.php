<div>
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500">
                    ...
                    <span @class(['font-medium', 'text-green-600' => $siteCrawl->status === 'completed', 'text-blue-600' => $siteCrawl->status === 'running', 'text-red-600' => $siteCrawl->status === 'failed'])>{{ ucfirst($siteCrawl->status) }}</span>
    ...
    {{-- Navigation bar --}}
    <div class="mb-4 flex flex-wrap items-center gap-2" x-data="{ openMenu: null }">
        {{-- Data tab dropdowns --}}
        @foreach([
            'seo' => ['label' => 'SEO', 'tabs' => ['internal' => 'Internal', 'page_titles' => 'Page Titles', 'meta_desc' => 'Meta Description', 'h1' => 'H1', 'h2' => 'H2', 'content' => 'Content']],
            'tech' => ['label' => 'Technical', 'tabs' => ['response_codes' => 'Response Codes', 'canonicals' => 'Canonicals', 'directives' => 'Directives', 'hreflang' => 'Hreflang', 'structured_data' => 'Structured Data', 'security' => 'Security', 'sitemaps' => 'Sitemaps']],
            'res' => ['label' => 'Resources', 'tabs' => ['images' => 'Images', 'links' => 'Links', 'external' => 'External Links', 'javascript' => 'JavaScript', 'css' => 'CSS']],
        ] as $groupKey => $group)
            @php $isActiveGroup = in_array($dataTab, array_keys($group['tabs'])); @endphp
            <div class="relative">
                <button @click="openMenu = openMenu === '{{ $groupKey }}' ? null : '{{ $groupKey }}'" @class([
                    'inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-medium transition',
                    'bg-purple-100 text-purple-700' => $isActiveGroup && $analysisTab === 'overview',
                    'bg-white border border-gray-300 text-gray-700 hover:bg-gray-50' => !$isActiveGroup || $analysisTab !== 'overview',
                ])>
                    {{ $group['label'] }}
                    @if($isActiveGroup && $analysisTab === 'overview')
                        <span class="text-purple-500">: {{ $group['tabs'][$dataTab] ?? '' }}</span>
                    @endif
                ...
                <div x-show="openMenu === '{{ $groupKey }}'" @click.outside="openMenu = null" x-cloak x-transition class="absolute left-0 z-30 mt-1 w-48 rounded-lg border border-gray-200 bg-white py-1 shadow-lg">
                    @foreach($group['tabs'] as $key => $label)
                        <button wire:click="setDataTab('{{ $key }}')" @click="openMenu = null" @class([
        ...
        @foreach(\App\Livewire\Seo\CrawlerResults::ANALYSIS_TABS as $key => $label)
            <button wire:click="setAnalysisTab('{{ $key }}')" @class([
                ...
                'bg-purple-600 text-white' => $analysisTab === $key && $key !== 'overview',
                'text-gray-600 hover:bg-gray-100' => $analysisTab !== $key || $key === 'overview',
            ])>{{ $label }}</button>
        @endforeach
                                ...
                                @foreach($columns as $col)
                                    <td class="whitespace-nowrap px-4 py-2.5">
                                        ...
                                        @elseif(in_array($col, ['title', 'meta_description', 'anchor', 'alt']))
                                            <span class="text-gray-600 max-w-sm truncate block">{{ \Illuminate\Support\Str::limit(is_string($val) ? $val : '', 55) }}</span>
