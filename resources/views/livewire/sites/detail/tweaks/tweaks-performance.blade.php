<div>
    <x-ui.page-header title="{{ __('Performance') }}" subtitle="{{ __('Optimize WordPress performance settings') }}">
        <x-slot:actions>
    ...
    <x-ui.card class="mb-6">
        <h3 class="text-base font-semibold text-gray-900 mb-4">{{ __('Heartbeat Control') }}</h3>
        <div class="space-y-3">
            <div class="flex items-center justify-between rounded-lg border border-gray-100 p-3">
                <div class="min-w-0">
                    <p class="text-sm font-medium text-gray-900">{{ __('Enable Heartbeat Control') }}</p>
                    <p class="text-xs text-gray-500">{{ __('Control the WordPress heartbeat API to reduce server load.') }}</p>
    ...
    {{-- Revisions Control --}}
    <x-ui.card class="mb-6">
        <h3 class="text-base font-semibold text-gray-900 mb-4">{{ __('Revisions Control') }}</h3>
        <div class="space-y-3">
            <div class="flex items-center justify-between rounded-lg border border-gray-100 p-3">
                <div class="min-w-0">
                    <p class="text-sm font-medium text-gray-900">{{ __('Limit Post Revisions') }}</p>
                    <p class="text-xs text-gray-500">{{ __('Control how many revisions WordPress keeps for each post.') }}</p>
    ...
    <x-ui.card class="mb-6">
        <h3 class="text-base font-semibold text-gray-900 mb-4">{{ __('Image Upload Control') }}</h3>
        <div class="space-y-3">
            <div class="flex items-center justify-between rounded-lg border border-gray-100 p-3">
                <div class="min-w-0">
                    <p class="text-sm font-medium text-gray-900">{{ __('Enable Image Optimization') }}</p>
                    <p class="text-xs text-gray-500">{{ __('Auto-resize uploaded images and control JPEG quality.') }}</p>
    ...
    {{-- Disable Components --}}
    <x-ui.card>
        <h3 class="text-base font-semibold text-gray-900 mb-4">{{ __('Disable Unnecessary Components') }}</h3>
        <div class="space-y-3">
            @php
                $components = [
                    'disable_emojis' => ['label' => __('Disable Emojis'), 'desc' => __('Remove WordPress emoji scripts and styles from the frontend.')],
                    'disable_dashicons' => ['label' => __('Disable Dashicons'), 'desc' => __('Remove Dashicons CSS on the frontend for non-logged-in users.')],
                    'disable_jquery_migrate' => ['label' => __('Disable jQuery Migrate'), 'desc' => __('Remove the jQuery Migrate script from the frontend.')],
                    'disable_generator_tag' => ['label' => __('Disable Generator Tag'), 'desc' => __('Remove the WordPress version meta tag from the page source.')],
                    'disable_wlw_manifest' => ['label' => __('Disable WLW Manifest'), 'desc' => __('Remove the Windows Live Writer manifest link.')],
                    'disable_rsd_link' => ['label' => __('Disable RSD Link'), 'desc' => __('Remove the Really Simple Discovery link from the header.')],
