<div>
    <x-ui.page-header title="{{ __('CAPTCHA') }}" subtitle="{{ __('Configure CAPTCHA for WordPress forms') }}">
        <x-slot:actions>
    ...
    <x-ui.card>
        <h3 class="text-base font-semibold text-gray-900 mb-4">{{ __('CAPTCHA Configuration') }}</h3>

        <div class="space-y-4">
            {{-- Provider --}}
            <x-ui.form-group label="{{ __('CAPTCHA Provider') }}" for="provider">
                <x-ui.select id="provider" wire:model.live="provider">
                    <option value="none">{{ __('None (disabled)') }}</option>
            ...
            @if($provider !== 'none')
                ...
                <div class="rounded-lg border border-gray-200 bg-gray-50 p-4 space-y-4">
                    <h4 class="text-sm font-medium text-gray-900">{{ __('API Keys') }}</h4>

                    @if($this->hasExistingKeys)
                        <p class="text-xs text-gray-500">{{ __('Keys are saved. Leave blank to keep existing keys, or enter new values to replace them.') }}</p>
                    @endif

                    <x-ui.form-group label="{{ __('Site Key') }}" for="siteKey" error="{{ $errors->first('siteKey') }}">
                        <x-ui.input type="text" id="siteKey" wire:model.live.debounce.500ms="siteKey"
                            placeholder="{{ $this->hasExistingKeys ? '••••••••••••' : __('Enter site key') }}" />
                    </x-ui.form-group>

                    <x-ui.form-group label="{{ __('Secret Key') }}" for="secretKey" error="{{ $errors->first('secretKey') }}">
                        <x-ui.input type="password" id="secretKey" wire:model.live.debounce.500ms="secretKey"
                            placeholder="{{ $this->hasExistingKeys ? '••••••••••••' : __('Enter secret key') }}" />
                    </x-ui.form-group>
                </div>

                {{-- Forms --}}
                <div class="space-y-3">
                    <h4 class="text-sm font-medium text-gray-900">{{ __('Enable on Forms') }}</h4>

                    <label class="flex items-center gap-2">
                        <x-ui.checkbox wire:model.live="enableLogin" />
                        <span class="text-sm text-gray-700">{{ __('Login form') }}</span>
                    </label>

                    <label class="flex items-center gap-2">
                        <x-ui.checkbox wire:model.live="enableRegister" />
                        <span class="text-sm text-gray-700">{{ __('Registration form') }}</span>
                    </label>

                    <label class="flex items-center gap-2">
                        <x-ui.checkbox wire:model.live="enableResetPassword" />
                        <span class="text-sm text-gray-700">{{ __('Password reset form') }}</span>
                    </label>

                    <label class="flex items-center gap-2">
                        <x-ui.checkbox wire:model.live="enableComments" />
                        <span class="text-sm text-gray-700">{{ __('Comment form') }}</span>
                    </label>
                </div>
            @endif
