<div @if($this->activeBackup || $awaitingBackup) wire:poll.2s="refreshProgress" @endif>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        {{-- Last Backup --}}
        <x-ui.card>
            <h3 class="text-base font-semibold text-gray-900 mb-4">{{ __('Last Backup') }}</h3>
            @if($this->lastBackup)
            ...
            @if($this->config->is_enabled)
                <div class="space-y-2 text-sm">
                    ...
                    <div class="flex justify-between">
                        ...
                        <span class="text-gray-900 font-medium">{{ $this->config->retention_value }} {{ $this->config->retention_type === 'count' ? __('backups') : __('days') }}</span>
                    </div>
                </div>
            @else
                <div class="space-y-2 text-sm">
                    ...
                    <p class="text-gray-500 text-xs mt-2">{{ __('Enable scheduled backups in the configuration below.') }}</p>
                </div>
            @endif
        </x-ui.card>
        ...
        {{-- Storage Usage --}}
        <x-ui.card>
            <h3 class="text-base font-semibold text-gray-900 mb-4">{{ __('Storage') }}</h3>
            <div class="space-y-2 text-sm">
                <div class="flex justify-between">
    ...
    <div class="mb-6" x-data="{ open: false }">
        <x-ui.card>
            <div class="flex items-center justify-between cursor-pointer" @click="open = !open">
                <h3 class="text-base font-semibold text-gray-900">{{ __('Backup Configuration') }}</h3>
            ...
            <div x-show="open" x-cloak x-transition class="mt-4">
                <form wire:submit="saveConfig" class="space-y-4">
                    {{-- Enable toggle --}}
                    <x-ui.checkbox wire:model.live="isEnabled" label="{{ __('Enable scheduled backups') }}" />
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        {{-- Backup type --}}
                        <div>
                            <label class="block text-sm font-medium text-gray-700">{{ __('Backup Type') }}</label>
                        ...
                        {{-- Storage destination --}}
                        <div>
                            <label class="block text-sm font-medium text-gray-700">{{ __('Storage Destination') }}</label>
                        ...
                        {{-- Retention type --}}
                        <div>
                            <label class="block text-sm font-medium text-gray-700">{{ __('Retention Policy') }}</label>
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-xs text-gray-400">{{ $retentionType === 'count' ? __('Number of backups to keep') : __('Days to retain backups') }}</p>
    ...
    <x-ui.card>
        ...
        @if($backups->isEmpty())
            <p class="text-sm text-gray-500 py-4 text-center">{{ __('No backups yet. Create your first backup using the button above.') }}</p>
        @else
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead>
                        <tr>
                            ...
                            <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">{{ __('Components') }}</th>
                            <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">{{ __('Size') }}</th>
                            <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase">{{ __('Duration') }}</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @foreach($backups as $backup)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-3 py-3 text-sm text-gray-700">{{ $backup->storageDestination?->name ?? __('Local') }}</td>
    ...
    <x-ui.modal name="create-backup" maxWidth="md">
        ...
        <div class="mt-4 space-y-4">
            ...
            <div class="space-y-2">
                <label class="block text-sm font-medium text-gray-700">{{ __('Additional Components') }}</label>
    ...
    <x-ui.modal name="restore-confirm" maxWidth="md">
        ...
        <div class="mt-2 rounded-lg bg-red-50 border border-red-200 p-3">
            <p class="text-sm text-red-800 font-medium">{{ __('Warning: This will replace your current database!') }}</p>
            <p class="text-sm text-red-700 mt-1">{{ __('This action will overwrite all current data with the backup contents. This cannot be undone. Make sure you have a recent backup before proceeding.') }}</p>
        </div>

        <div class="mt-4">
            <label class="flex items-center gap-2">
                <input type="checkbox" wire:model="restoreConfirmed" class="rounded border-gray-300 text-red-600 focus:ring-red-500">
                <span class="text-sm text-gray-700">{{ __('I understand this will overwrite my current database') }}</span>
            </label>
        </div>
            ...
            @else
                <div class="grid grid-cols-3 gap-3 text-center">
                    <div class="rounded-lg bg-gray-50 p-3">
                        ...
                        <div class="text-xs text-gray-500">{{ __('Tables checked') }}</div>
                    </div>
                    <div class="rounded-lg bg-green-50 p-3">
                        <div class="text-lg font-bold text-green-700">{{ $restoreVerification['tables_matched'] ?? 0 }}</div>
                        <div class="text-xs text-green-600">{{ __('Matched') }}</div>
                    </div>
                    <div class="rounded-lg {{ ($restoreVerification['tables_different'] ?? 0) > 0 ? 'bg-yellow-50' : 'bg-gray-50' }} p-3">
                        ...
                        <div class="text-xs {{ ($restoreVerification['tables_different'] ?? 0) > 0 ? 'text-yellow-600' : 'text-gray-500' }}">{{ __('Different') }}</div>
                    </div>
                </div>

                @if(!empty($restoreVerification['details']))
                    <div class="rounded-lg border border-yellow-200 bg-yellow-50 p-3">
                        <h4 class="text-xs font-semibold text-yellow-800 mb-2">{{ __('Tables with differences:') }}</h4>
                        <div class="space-y-1">
                        ...
                        <p class="mt-2 text-xs text-yellow-700">{{ __('Minor differences in tables like activity_logs or app_backups are expected, as the restore itself creates new records.') }}</p>
                    </div>
                @endif
            @endif
        </div>
    ...
    <x-ui.modal name="view-log" maxWidth="lg">
        ...
        <div class="mt-3 max-h-96 overflow-y-auto rounded-lg border border-gray-200 bg-gray-900 p-4">
            ...
            @empty
                <p class="text-gray-500 text-xs">{{ __('No log entries available.') }}</p>
            @endforelse
        </div>
        <div class="mt-4 flex justify-end">
