...
{{-- Administrator table --}}
@if(!empty($wu['admin_list']))
    <h3>{{ __('report.wp_admin_list', [], $lang) }}</h3>
    <table class="data-table">
        ...
        <tbody>
            @foreach($wu['admin_list'] as $admin)
                <tr>
                    <td>{{ $admin['username'] }}</td>
                    <td class="cell-break">{{ $admin['email'] }}</td>
                    <td>{{ $admin['last_login_at'] ?? __('report.wp_never', [], $lang) }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
