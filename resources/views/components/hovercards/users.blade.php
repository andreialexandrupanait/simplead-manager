...
@if($usersCount > 0)
    <div class="flex items-center justify-between">
        <span class="text-sm font-semibold text-gray-900 dark:text-gray-100">WordPress Users</span>
        <x-ui.badge variant="blue">{{ $usersCount }}</x-ui.badge>
    </div>
    <div class="mt-3 space-y-1.5 text-xs">
        <div class="flex items-center justify-between">
            <span class="text-gray-500 dark:text-gray-400">Total users</span>
            <span class="font-medium text-gray-900 dark:text-gray-100">{{ $usersCount }}</span>
        </div>
        @if($site->users_last_synced_at)
            <div class="flex items-center justify-between">
                <span class="text-gray-500 dark:text-gray-400">Last synced</span>
                <span class="font-medium text-gray-900 dark:text-gray-100">{{ $site->users_last_synced_at->diffForHumans() }}</span>
            </div>
        @endif
    </div>
...
@else
    <p class="text-sm text-gray-500 dark:text-gray-400">No users synced</p>
