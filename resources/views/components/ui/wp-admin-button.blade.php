...
<div class="inline-flex items-center">
    ...
    <button wire:click="openWpAdmin"
            ...
            class="inline-flex items-center gap-2 rounded-l-lg border border-gray-300 bg-white px-3 py-1.5 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 transition disabled:opacity-50 disabled:cursor-not-allowed {{ !$hasAdmins ? 'rounded-r-lg' : 'border-r-0' }}">
        ...
        <span wire:loading.remove wire:target="openWpAdmin">
            WP Admin
            @if($selectedUser)
                <span class="text-gray-400 font-normal">as {{ $selectedUser->display_name ?: $selectedUser->username }}</span>
            @endif
        </span>
    ...
    @if($hasAdmins)
        <x-ui.dropdown align="right" width="56">
            <x-slot:trigger>
                <button type="button"
                        class="inline-flex items-center rounded-r-lg border border-gray-300 bg-white px-2 py-2 text-gray-500 shadow-sm hover:bg-gray-50 transition">
                    <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="px-3 py-2 text-xs font-semibold text-gray-400 uppercase tracking-wider">Login as</div>
                ...
                <button wire:click="setWpAdminUser({{ $admin->id }})"
                    ...
                    <div class="text-left truncate">
                        ...
                        @if($admin->display_name && $admin->display_name !== $admin->username)
                            <span class="text-gray-400 text-xs">({{ $admin->username }})</span>
                        @endif
                    </div>
                </button>
            @endforeach
