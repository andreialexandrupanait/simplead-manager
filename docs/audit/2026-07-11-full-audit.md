...
**Storage (2026-07-11):** the primary object store is **Hetzner Object Storage (S3-compatible)** — the existing `s3` disk already supports a custom `endpoint` (`config/filesystems.php:57`), so presigned multipart + content-addressed blobs work unchanged against the Hetzner bucket (secondary Dropbox/other kept as an optional off-provider replica for worst-case redundancy).

**Why this design specifically meets "no stress on the client server" (owner's hard requirement):** the client site NEVER compresses, never stages a zip, never holds an archive, and never runs a long process — it only reads changed files/DB-chunks and streams small units out, each in a sub-`max_execution_time` request. All CPU-heavy work (compression, dedup, synthetic-full assembly, restore orchestration) happens on the SimpleAd manager + Hetzner side. This is exactly the BlogVault/WPMU "zero-footprint on the client" property, and it's the whole reason the thin-client model beats trying to make on-site full backups faster.
